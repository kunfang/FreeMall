...
public class WeiXinSignAndPackage {
	...
	public static String createPackage(TreeMap<String,String> treeMap){
		...
		String stringSignTemp = string1 + "key="+DefaultParams.findParam("paternerKey");
		String sign=MD5Util.MD5Encode(stringSignTemp, DefaultParams.findParam("input_charset")).toUpperCase();		
		return sign;
	}
	/**
	 * 创建支付签名paysign
	...
	public static String createPaySign(TreeMap<String,String> param){
		...
		String paysign=MD5Util.MD5Encode(stringSignTemp, DefaultParams.findParam("input_charset")).toUpperCase();
		return paysign;
	}
	/**
	 * wxpackage组装原始串
	...
	private static String originalURLString(TreeMap<String,String> treeMap){
		...
		try {
			for (Entry<String, String> obj : entry) {
				...
				if (v == null && v.equals(""))
					continue;
				sb.append(k.toLowerCase() + "=" + URLEncoder.encode(v, DefaultParams.findParam("input_charset")) + "&");
			}
		} catch (Exception e) {
	...
	public static String getPrepayId(TreeMap<String, String> payParamMap){
		String result=HttpTool.sendPost(DefaultParams.findParam("prepay_id_url"), payParamMap, DefaultParams.findParam("input_charset"));
		return result;
	}
}
