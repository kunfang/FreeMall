...
var $d=null;
function DatePicker(){if(parent==window)return;
if(navigator.product!='Gecko'){
...
DatePicker.prototype.init=function(){this._todayDate=new Date();
...
DatePicker.prototype._setShowAndHide=function(){if(!(this.hasYear||this.hasMonth)){this.dd.titleDiv.style.display='none';}
...
DatePicker.prototype._setActiveCSS=function(){var i,a,main;
...
DatePicker.prototype.redraw=function(){this.year=this.dd.yInput.value;
this.month=this.dd.mInput.getAttribute("realValue");
this.dd.dDiv.innerHTML=this._createDateTable();
autoSizeIframe();};
DatePicker.prototype._initDate=function(str,fmt)
...
DatePicker.prototype._isDate=function(sDate){return sDate.match(/^((\d{2}(([02468][048])|([13579][26]))[\-\/\s]?((((0?[13578])|(1[02]))[\-\/\s]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\-\/\s]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\-\/\s]?((0?[1-9])|([1-2][0-9])))))|(\d{2}(([02468][1235679])|([13579][01345789]))[\-\/\s]?((((0?[13578])|(1[02]))[\-\/\s]?((0?[1-9])|([1-2][0-9])|(3[01])))|(((0?[469])|(11))[\-\/\s]?((0?[1-9])|([1-2][0-9])|(30)))|(0?2[\-\/\s]?((0?[1-9])|(1[0-9])|(2[0-8]))))))(\s(((0?[0-9])|([1-2][0-3]))\:([0-5]?[0-9])((\s)|(\:([0-5]?[0-9])))))?$/);};
DatePicker.prototype._createPickerHTML=function()
{var s=new __sb();
s.a("<div id=dpTitle>");
...
s.a("</div>");
return s.j();};
DatePicker.prototype._createDateTable=function(){var tempYear,tempMonth;
...
s.a("</table>");
return s.j();};
DatePicker.prototype._createTimeTable=function(){var s=new __sb();
...
s.a("</div>");
return s.j();};
DatePicker.prototype._fillmonth=function()
{var s=new __sb();
...
DatePicker.prototype._fillyear=function(minV,maxV)
...
DatePicker.prototype._setOkInput=function(){this.dd.clearInput.onclick=function(){$d.obj.eCont.value='';};
...
DatePicker.prototype._returnDateStr=function(Y,M,D,h,m,s,fmt)
...
return sDate;};
DatePicker.prototype._doStr=function(s,len){s=s+'';
for(var i=s.length;i<len;i++){s='0'+s;}
return s;};
DatePicker.prototype._setRealValue=function(Y,M,D,h,m,s){if(this.showTime){this.eCont.setAttribute("REALVALUE",this._returnDateStr(Y,M,D,h,m,s,dpcfg.realValueLongFmt));}
else{this.eCont.setAttribute("REALVALUE",this._returnDateStr(Y,M,D,null,null,null,dpcfg.realValueShortFmt));}};
DatePicker.prototype.pickDate=function(Y,M,D,h,m,s){if(Y==null){Y=this.dd.yInput.value;}
...
this.onPicked();};
DatePicker.prototype.modifyDate=function(Y,M,D,h,m,s){if(Y==null){Y=this.dd.yInput.value;}
if(M==null){M=this.dd.mInput.getAttribute("realValue");}
if(D==null){D=this.date;}
this.year=Y;this.month=M;this.date=D;
if(this.showTime){if(h==null){h=this.dd.hhInput.value;}
...
this.pickDate(Y,M,D);}
DatePicker.prototype._markValue=function(bValue){if(bValue){this.eCont.className=this.eCont.className.replace(/ WdateFmtErr/,'');}
...
break;}
break;}}};
DatePicker.prototype._judgeCorrectDateTime=function(sDateTime){var Y,M,D,h,m,s;
...
return false;};
DatePicker.prototype._doCustomDate=function(str,fmt,dvType){var defaultValue=(dvType=="min")?dpcfg.minDate:dpcfg.maxDate;
