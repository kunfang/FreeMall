...
<body>
	<div class="pd_box-border">
		<div class="pd_product-title-wrap">
                <div class="pd_product-title-box">
                    <h1 class="pd_product-title" id="pd_product-title" style="text-align: center;font-size：2.5rem;"><span style="color: red">[免费领取]</span><%=prodname %></h1>
                </div>               
            </div>
	</div>
	...
	<li><input type="submit" class="save" onclick="save()" value="下一步" id="submitAddress"></li>
	</ul>
	</div>
	</div>
...
<script type="text/javascript">
	...
	function save(){
		...
		var receiverName=document.cform.receiverName.value;
		var regionCode=document.cform.regionCode.value;
		var cityCode=document.cform.cityCode.value;
		var countyCode=document.cform.countyCode.value;
		var address=document.cform.address.value;
		var receiverMobile=document.cform.receiverMobile.value;

		var url ="<%=basePath%>OAuthGetCode?prodID=<%=prodid%>&prodName=<%=prodname%>&price=<%=price%>&agentID=<%=agentid%>"+
			"&receiverName="+receiverName+"&regionCode="+regionCode+"&cityCode="+cityCode+"&countyCode="+countyCode
			+"&address="+address+"&receiverMobile="+receiverMobile;
